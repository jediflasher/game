package ru.catAndBall.data.game {
	...
	public class GridFieldSettings {
		...
		public var startElements:Vector.<String> = new Vector.<String>();

		// список шансов выпадения элементов стартового, порядок соответствует списку типов
		public var startElementChances:Vector.<Number> = new Vector.<Number>();
		...
		public function deserialize(input:Object):void {
			...
			if ('elements' in input) ArrayUtils.toVector(input.elements, elements);
			if ('elementChances' in input) ArrayUtils.toVector(input.elementChances, elementChances);
			if ('startElements' in input) ArrayUtils.toVector(input.startElements, startElements);
			if ('startElementChances' in input) ArrayUtils.toVector(input.startElementChances, startElementChances);
			if ('pests' in input) {
