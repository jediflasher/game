package ru.catAndBall.view.core.game {
	...
	public class Building extends Sprite {
		...
		public function Building(data:BuildingData, assetId:String) {
			super();
			this.data = data;
			_assetId = assetId;
		...
		private var _construction:Boolean = false;
		...
		public override function dispose():void {
			data.removeEventListener(BuildingData.EVENT_BUILDING_COMPLETE, handler_buildingComplete);
			if (_construction) Starling.juggler.remove(_constructionProgress);
		...
		private function update(event:Event = null):void {
			_construction = data.startBuildingTime > 0;
		...
		private function updateHandlers():void {
			if (data.startBuildingTime) {
				data.addEventListener(BuildingData.EVENT_BUILDING_COMPLETE, update);
			} else {
				data.removeEventListener(BuildingData.EVENT_BUILDING_COMPLETE, update);
			}

			if (data.bonusTimeLeft > 0) {
				data.addEventListener(BuildingData.EVENT_BONUS_TIME_COMPLETE, update);
			} else {
				if (data.canCollectBonus) {
					data.addEventListener(BuildingData.EVENT_BONUS_COLLECTED, update);
				} else {
					data.removeEventListener(BuildingData.EVENT_BONUS_COLLECTED, update);
				}
				data.removeEventListener(BuildingData.EVENT_BONUS_TIME_COMPLETE, update);
			}
		}
